...
function addFlavor(arrayToEdit,flavorToAdd){
  arrayToEdit.unshift(flavorToAdd)
  return arrayToEdit
  }
...
function removeLastFlavor(arrayToEdit){
 arrayToEdit.pop()
 return arrayToEdit
}
...
function getFlavorByIndex(arrayToSearch,searchIndex){
  return arrayToSearch[searchIndex]
}
...
function removeFlavorByName(arrayToEdit,flavorName){
  arrayToEdit.splice(arrayToEdit.indexOf(flavorName),1)
  return arrayToEdit
}
...
function filterByWord(arrayToFilter,searchString){
  const newArr = arrayToFilter.filter(item => item.includes(searchString))
  return newArr
}
...
function getAverageWordLength(originalFlavors){
  let totalWordsSum = 0
  const wordsPerItemArr = originalFlavors.map(item => item.split(" ").length)
  for( let i = 0; i < wordsPerItemArr.length ; i++){
    totalWordsSum += wordsPerItemArr[i]
  }
  return totalWordsSum/wordsPerItemArr.length
}
...
function getRandomFlavors(arr1,arr2,arr3,arr4){
  ...
  let randomFlavors = []
  for(let i = 0; i < 31 ; i++){
    randomFlavors.push(allFlavorsArr[Math.floor(Math.random() * allFlavorsArr.length)])
  }
